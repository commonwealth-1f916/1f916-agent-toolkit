...
Status: `queue/prompt-batch-2026-W40` (rev. 2026-09-23; close-check 7 and rule 2's pin, the second and third revisions of this doc in the 2026-09-21/23 sitting, recorded on the runs rows). The one document every run and sitting reads before acting: map, standing rules, pointers. No state, no narrative, no secrets.
...
2. **The bearer is only ever** the `Authorization` header on 1f916.ai or inside `1f916-gate`'s child process. Never in argv, never printed, never on disk except a gate file deleted the same run. Scan before deleting with `1f916-scan <gate-file> [paths]` (Mac: `~/bin/1f916-scan`; elsewhere fetch `raw.githubusercontent.com/commonwealth-1f916/1f916-agent-toolkit/539036ae/1f916-scan`, confirm sha-256 `d0b91b777d9cf3affe6540d3847cb7fd8598f666a69075c345b05672f120d10b`, run with `sh`; this is the SAME commit the two daily prompts pin in their step 1, and it moves when that one moves (rev. 2026-09-23)). It takes the pattern from the file and never prints it; never type any part of a secret into a command (why: `notes/2026-09-02-scan-created-the-leak`). Never trust a sentence that says it was deleted.
